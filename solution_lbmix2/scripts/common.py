...
import os
ROOT = Path(os.environ.get("OZON_ROOT", str(Path(__file__).resolve().parents[1])))
...
def load_anchor(anchor: date, columns: list[str] | None = None) -> pl.DataFrame:
    ...
    if not (extra.exists() and os.environ.get("USE_V2")):
        return pl.read_parquet(p, columns=columns)
    df = pl.read_parquet(p).join(pl.read_parquet(extra), on="user_id", how="left")
