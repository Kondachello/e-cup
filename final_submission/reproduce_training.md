...
**Остальные двенадцать членов — прогноз-артефакты.** Копии их тестовых прогнозов
лежат в `final_submission/models/preds_test/` и используются инференсом как кэш;
чтобы получить их обучением, нужен полный прогон:
