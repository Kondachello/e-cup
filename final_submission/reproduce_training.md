...
Документ описывает **фактический** пайплайн действующего решения — той самой базы, из
которой собираются оба выбранных финальными файла, `F14_int08` и `F21_seg4g08`
(`final_submission/assemble_finals.py`, `deltas/README.md`).
...
Замерено и на сеточных семействах вне действующего состава: у `mlpziln` средняя дельта
−0.000612 на сид, интервал [−0.000909, −0.000296]; сырой критерий останавливал на
эпохе 1–4 вместо 6–7.

В командах раздела 3 флаг стоит **ровно там, где он был при обучении**. Ставить его
...
Ровно эти четыре команды выполняет `inference.py --stage features`.
...
| v4 | `USE_V4` | BTYD: BG/NBD (P(alive), ожидаемое число покупок) + Gamma-Gamma (чек) | всех, кроме `c_ts2_*` |
| seq3 | — | uint8 [250k × 112 дней × 12 каналов] на якорь | `fusion_v3*` |

Контрольные числа колонок: **203** при V2+V3+V4, **194** при V2+V3,
**85** у `behavonly` (правило отбрасывания «денежных» колонок).

> Тиры v5/v6/v7/v8/v10 и тензоры seq2 в действующем составе не читает ни один член:
> ни один из 18 базовых прогнозов не выставляет соответствующий `USE_*`. Стадия
> `features` их не собирает, и в этом репозитории их сборщиков нет. Исключение —
> `build_features_v5.py`: его функции импортирует `train_wklin.py`, поэтому файл
> оставлен; `--emit-tier` в команде wklin попутно пишет `.v5s.parquet`, сам бленд его
> не использует, но флаг сохранён, потому что модель обучалась именно так.

## 3. Шаг 1: базовые модели

Команды ниже воспроизводят обучение каждой модели и исторически точны. Описаны только
семейства, вошедшие в действующий состав: подразделы моделей, получивших при
переоптимизации вес ровно ноль (`seq2tr_f`, `mlpziln_c*`, `countaov_s7`, `hmmsim`,
`c_xtw_s42`, `twl_v7`), убраны вместе с их трейнерами — см. §9.

| подраздел | модель | вес в бленде |
|---|---|---|
| 3.1 | `fusion_v3c*`, `fusion_v3ctl`, `fusion_f` | **0.346** |
| 3.2 | `c_ts2_s42` | **0.033** |
| 3.3 | `behavonly` (3 сида) | **0.041** |
| 3.4 | `weak_an_d`, `weak_ft_recency` | **0.089** (`weak_ft_counts`, `weak_ft_long90` — вес 0) |
| 3.5 | `wklin`, `wklin_wk` | **0.074** |
| 3.6 | `febspec2` | **0.009** |
...
### 3.2 Бустинги, `train_gbdt.py` и обёртки над ним
...
В бленд из этого семейства вошёл только сид 42; сид 7 обучается той же командой и
в состав не попал.

### 3.3 Модель без единого «денежного» признака, `train_behavonly.py`
...
### 3.4 Слабые специализированные модели, `train_weak.py`
...
### 3.5 Линейная модель на недельных колонках, `train_wklin.py`
...
### 3.6 Февральский специалист, `train_febspec2.py`
...
**Шаг 4 — приведение к моментам.** Оценено отправками: пара файлов, отличающихся на
известную константу в log-пространстве, даёт точное среднее из разности квадратов
скоров. Так получены среднее log1p **2.3247** и разброс **1.6320**.
...
пересчитать их из весов нельзя. Поэтому они входят в пакет как данные: замороженными в
`final_submission/models/chain_test.npz`, вместе со старым направлением поправки на
молчащих (`dir_old`, раздел 7). Стадии, которая бы их пересобирала, в пакете нет —
пересобирать их не из чего.
...
Живое состояние печатает одна команда — она же даёт команду восстановления для
каждого недостающего артефакта и является источником истины, а не эта таблица:

```bash
.venv/bin/python final_submission/inference.py --stage check
```

Что в пакете есть, а что нет, по состоянию отгрузки.

**Веса лежат в `final_submission/models/` — инференс их грузит (6 моделей):**
`c_ts2_s42`, `behavonly`, `behavonly_s1337`, `behavonly_s7`, `weak_an_d`,
`weak_ft_recency`, плюс двенадцать таблиц калибровки и замороженная цепочка
`chain_test.npz`. Их суммарный вклад в бленд — 0.164 из 1.006.

**Остальные двенадцать членов — прогноз-артефакты** (суммарный вес 0.842). Их
трейнеры не сохраняют веса, поэтому проверяемый артефакт у них один — тестовый
прогноз; копии лежат в `final_submission/models/preds_test/`, приезжают вместе с
репозиторием, и инференс берёт их оттуда сам. Всё ниже нужно, только если проверять
эти прогнозы обучением, а не принимать их:

| модель | вес | чем воспроизводится | время |
|---|---|---|---|
| `kostya46` | 0.246 | `python work_kostya/reproduce.py`, сверка `check_reproduce.py` | ~1.5 ч |
| `fusion_v3c42`, `fusion_v3c555`, `fusion_v3c7`, `fusion_v3ctl` | 0.336 | `train_fusion3.py` — §3.1; нужны тензоры `work/seq3` | 15–18 мин каждая |
| `gseq_small_s42` | 0.109 | Colab GPU, `work/colab/gpu_seq.py`, конфиг `work/colab/out/gseq_small_s42.json` | ~3 ч |
| `gseq_big_s42` | 0.024 | то же, рука `big`; прогон был оборван на шаге 4500 из 11736 — **точное повторение невозможно** | ~3 ч |
| `wklin`, `wklin_wk` | 0.074 | `train_wklin.py` — §3.5, один прогон даёт оба | 1.5 мин |
| `lagd28` | 0.035 | `lag_tta.py`; воспроизводится **побитово** | 13 мин |
| `febspec2` | 0.009 | `train_febspec2.py` — §3.6 | 7 мин |
| `fusion_f` | 0.011 | тензоры `work/seq2` (11 ГБ) удалены и стадией `features` больше не собираются — **повторное обучение невозможно**, артефактом остаётся прогноз | — |

Плюс сборка признаков и тензора `seq3` (3.4 ГБ), недостающие таблицы калибровки
(~1 мин каждая) и обучение модели молчания (~10 мин). `--stage check` на чистой
машине оценивает всё вместе примерно в **9 часов последовательного обучения**.

> **Самое дешёвое улучшение воспроизводимости**, если появится время: добавить вызовы
> `model_io.save_torch` / `save_meta` в `train_fusion3.py`. Это закрыло бы дыру на
> 0.336 веса бленда — треть решения сейчас нельзя проверить, не переобучив.

## 9. Что НЕ входит в решение

Репозиторий приведён к тому, чем собираются `F14_int08` и `F21_seg4g08`, поэтому
перечисленного ниже в нём **нет**: оно проверено, измерено и отвергнуто, и тратить
время на его воспроизведение не нужно.

**Наборы признаков и тензоры:** v5, v6, v7, v8, v10 и тензоры seq2. Ни один из 18
базовых прогнозов не выставляет соответствующий `USE_*`; стадия `features` их не
собирает. `build_features_v5.py` и `build_features_v10.py` в репозитории остались —
их функции импортируют `train_wklin.py` и `build_features_short.py`.

**Модели с весом ровно ноль в действующем составе:** `seq2tr_f` (секвенсная на
тензорах seq2), `mlpziln_c*` (сеточная ZILN), `countaov_s7` (разложение число × чек),
`hmmsim` (генеративный симулятор), `c_xtw_s42` (XGBoost tweedie), `twl_v7` (LightGBM
на колонках HMM). Их трейнеры убраны вместе с ветками инференса, которые их грузили.

**Отвергнутые направления:** `train_mlp.py`, `train_gru.py`, `train_bagged.py`,
`train_gls.py`, `train_pseudo.py`, `train_quantint.py`, `train_rank.py`,
`train_whale.py`, `train_horizon.py`, `train_hjit.py`, `train_channel.py`,
`train_mlpbin.py`, `train_feb_specialist.py`, `train_febspec3.py`, `train_xtw.py`.
Из этого ряда в репозитории оставлены только `train_fusion.py` (предшественник
`train_fusion3.py` на тензорах seq2 — им обучен член `fusion_f`) и `train_mlp2.py`
(его функции импортирует `short_family.py`, через который работает `train_febspec2.py`).
