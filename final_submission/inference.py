...
опорная оценка 1.6479652993) из train.parquet + sample_submit.csv. Сетевых вызовов нет.
...
ШЕСТЬ ШАГОВ РЕШЕНИЯ (порядок обязателен, каждый шаг — отдельная стадия ниже):
  ...
  4. Приведение к оценённым моментам целевого окна log1p: среднее REF_MEAN 2.3247,
...
# ============================================================================ #
# Моменты log1p прогноза, ОЦЕНЁННЫЕ ДЛЯ ЦЕЛЕВОГО ОКНА (KNOWLEDGE.md, факт Ф18):
...
def load_chain() -> dict:
    ...
    if p is None:
        raise MissingArtifact(
            f"НЕ ХВАТАЕТ ФАЙЛА: {CHAIN_NPZ} (оценённая цепочка поправок поправок)\n"
            f"  искали в: {MODELS_DIR}\n            {WORK_MODELS}\n"
