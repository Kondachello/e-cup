...
for c in cols:
    ...
    # НЕцентрированная корреляция — её требует тождество (та же ошибка, что я нашёл
    # в err_corr.py; ревью справедливо указало, что в этом скрипте она осталась)
    rho = float(np.mean(em*eb)/(sm*sb))
    r = sb/sm; d = r - rho
    dd = em - eb
...
for s_m in (1.6700, 1.6800, 1.7000, 1.7500, 1.8266):
    ...
    # ТОЧНОЕ решение: s_b*d^2 - 4*G*r*d - 2*G*(1-r^2) = 0. Отбрасывать член 2rd
    # нельзя — у сильных моделей он сравним с (1-r^2) и порог занижается на треть.
    G = 0.0003
    need = (4*G*r + np.sqrt((4*G*r)**2 + 8*G*sb*(1-r*r)))/(2*sb)
