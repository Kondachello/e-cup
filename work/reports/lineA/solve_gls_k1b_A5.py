"""solve_gls_k1b_A5.py — КОПИЯ solve_gls_k1b_A3.py для линии A (задача , LOO по группам).
...
def main() -> None:
    ...
    #LOO ПО ГРУППАМ ОСЕЙ ============================
    # Группы — семейства осей (см. a5_loo_groups.md). Выкидываем группу ЦЕЛИКОМ,
    # ПЕРЕОЦЕНИВАЕМ сегментный эмпирический приор на оставшихся точках и пересобираем
    # всю систему (shrinkage -> c_Q -> гребень -> дозы). Затем считаем честный
    # out-of-fold выигрыш группы: дозы взяты из ПОЛНОГО решения (как в реальной посылке),
    # а вклад группы в скор зачитывается по held-out ожиданию её каппы.
    # Ключевой факт: приор Sig_p ДИАГОНАЛЕН, поэтому cov(c_G, cP_{-G}) = 0 —
    # held-out предсказание каппы выкинутой группы РОВНО равно её приорному среднему.
...
if __name__ == "__main__":
    ...
    ap.add_argument("--loo", action="store_true", help=": LOO по группам осей")
    a = ap.parse_args()
    if a.prior_model:
