"""solve_gls_k1b_A4.py — КОПИЯ  для линии A (задача , СТОРОЖ).
ключ --keep-n1. Сегментный приор считается по ИСХОДНОМУ списку (names0), чтобы
сравнение «с досыпкой / без» было контролируемым. Оригинал НЕ ТРОНУТ.
...
EXTEND = "none"                       #none | z | dead | all
FIX_AXES = {}                         #{ось: значение Δдозы} — зажать
KEEP_N1 = False                       #не исключать mdl_wulfen
CENTER_EXT = True                     #центрировать досыпанные направления
EB_DECOMP = False                     #пересчитать decomp-приор по всем Z
...
def main() -> None:
    ...
    #ДОСЫПКА МЁРТВЫХ И Z-ОСЕЙ В ГРАМ ====================
    names0 = list(AX)                      # спан F8 (47 имён; mdl_wulfen исключается в финале)
    EXT_Z = []
    EXT_SEG = [("", "R9"), ("", "R9"),
               ("", "R9"), ("", "R9"),
               ("", "F1"), ("", "F1"),
               ...
               ("", "F3"), ("", "F3"),
               ("", "F3")]
    EXT_MODEL = [("mdl_kyanit", "E1_erafix_full", "T3", 1.0), ("", "V3", 0.5)]

    def add_ext(k, pf, bk, b_pr, fam):
        S[k] = MEAS[pf]

    if EXTEND in ("z", "all"):
        for z in EXT_Z:
            add_ext(z, z, "R8", 1.0, "decomp")
    if EXTEND in ("dead", "all"):
        for k_, pf_, bk_ in EXT_SEG:
            add_ext(k_, pf_, bk_, 1.0, "segment")
        for k_, pf_, bk_, b_ in EXT_MODEL:
            add_ext(k_, pf_, bk_, b_, "model")
    ext_names = [k for k in AX if k not in names0]
    if ext_names:
        print(f"\nA4: досыпано осей в Грам: {len(ext_names)} "
              f"(центрирование={CENTER_EXT})\n    {ext_names}")
    # =============================================================================
    ...
    for i, k in enumerate(names):
        a = AX[k]
        if a.get("ext"):                       #досыпанная ось
            pf, bk, b_pr = a["ext"]["pf"], a["base"], a["ext"]["b"]
            s_step = a["ext"]["step"]
            s_probe = MEAS[pf]
        elif a["probe"] is None:
    ...
    extra_seg = [("R9"), ("R9"), ("R9"),
                 ...
                 ("F3"), ("F3"), ("F3")]
    seg_pts = []
    for k in names0:                            #приор — по ИСХОДНОМУ спану
        if AX[k]["fam"] == "segment":
    ...
    #EB по decomp-семейству (все Z в системе) --------------------
    prior_decomp_used = PRIOR_DECOMP
    dec_eb = None
    dec_idx = [i for i, k in enumerate(names) if AX[k]["fam"] == "decomp"]
    if len(dec_idx) >= 3:
        kd = np.array([kap[names[i]] for i in dec_idx])
        sd_ = np.array([sig[names[i]] for i in dec_idx])
        dec_eb = eb_fit(kd, sd_, grid_hi=0.3, ngrid=3001)
        dec_eb["axes"] = [names[i] for i in dec_idx]
        print(f"\nA4: decomp-приор EB по {len(dec_idx)} Z-осям: MoM tau={dec_eb['tau_mom']:.4f}  "
              f"ML tau={dec_eb['tau_ml']:.4f} (mu {dec_eb['mu_ml']:+.4f})  "
              f"REML tau={dec_eb['tau_reml']:.4f}  CI95 "
              f"[{dec_eb['ci95'][0]:.4f},{dec_eb['ci95'][1]:.4f}]   "
              f"(реестр {PRIOR_DECOMP[0]:+.4f}/{PRIOR_DECOMP[1]:.4f})")
        if EB_DECOMP:
            prior_decomp_used = (float(dec_eb["mu_ml"]), float(dec_eb["tau_ml"]))
            print(f"  ПРИМЕНЁН EB decomp-приор: mu={prior_decomp_used[0]:+.4f}, "
                  f"tau={prior_decomp_used[1]:.4f}")

    PRIORS = {"model": prior_model_used, "segment": (MU_SEG, TAU_SEG),
              "decomp": prior_decomp_used}
    ...
    keep = [i for i, k in enumerate(names) if (k != "mdl_wulfen" or KEEP_N1)]
    ...
    A_fin = Qk + Lam_k + ridge_k + GAMMA_STAR * np.diag(np.diag(Qk))
    d_fin = np.linalg.solve(A_fin, cQ_k)
    if FIX_AXES:                                     #зажать оси на заданных значениях
        fi = [j for j, k in enumerate(kn) if k in FIX_AXES]
        fr = [j for j in range(len(kn)) if j not in fi]
        xv = np.array([float(FIX_AXES[kn[j]]) for j in fi])
        rhs = cQ_k[fr] - A_fin[np.ix_(fr, fi)] @ xv
        d_fin = np.zeros(len(kn))
        d_fin[fr] = np.linalg.solve(A_fin[np.ix_(fr, fr)], rhs)
        d_fin[fi] = xv
        print(f"\n  : ЗАЖАТЫ оси {FIX_AXES} (остальные пересчитаны при зажиме)")
    ...
    np.savez(OUT / f"gls_state{TAG}.npz", names=np.array(kn), Q=Qk, cQ=cQ_k, cP=cP_k,
             ...
             ext_names=np.array(ext_names if ext_names else [""]),
             fam=np.array([AX[k]["fam"] for k in kn]),
             kappa=np.array([kap[k] for k in kn]),
             sigma=np.array([sig[k] for k in kn]),
             gloc=np.array([g_vec[names.index(k)] for k in kn]),
             prior_decomp=np.array(prior_decomp_used),
             prior_seg=np.array([MU_SEG, TAU_SEG]))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--extend", default="none", choices=["none", "z", "dead", "all"],
                    help=": досыпать в Грам Z-оси / мёртвые / всё")
    ap.add_argument("--fix", default=None,
                    help=": зажать оси, напр. 'mdl_gneis2=0' или 'mdl_gneis2=0.2,mdl_malach=0'")
    ...
    EB_DECOMP = a.eb_decomp
    if a.fix:
        FIX_AXES = {kv.split("=")[0]: float(kv.split("=")[1]) for kv in a.fix.split(",")}
