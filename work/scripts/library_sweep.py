...
def main():
    ...
    for f in sorted(PREDS_DIR.glob("*_val.parquet")):
        ...
        if s < SANITY_FLOOR:
            ...
            continue
        # ПОРЯДОК ПРОВЕРОК ИСПРАВЛЕН 25.08. Близость соло-скора к бленду — грубая сеть, и
        # она отсекала кандидата ДО принципиальной проверки. На tfm4_cal (гибрид
        # последовательность+таблица, усреднение трёх сидов) это дало ложное срабатывание:
        # скор 1.6665 против бленда 1.6656, то есть ХУЖЕ бленда на 0.0008, а парный вклад
        # +0.00051 — вчетверо НИЖЕ потолка. Для сравнения, случай, ради которого сторож и
        # писался (direct_val2chk): соло ЛУЧШЕ бленда и парный вклад +0.016, в 35 раз выше
        # рекорда. Ни один признак не совпал, а модель выбрасывалась.
        # Настоящий детектор узкой утечки — парный вклад, он и остаётся жёстким. Близость
        # скора теперь только предупреждает, причём лишь когда парный вклад тоже повышен.
        g1 = pair_gain(lp)
        if s < sb_full + 0.001:
            if g1 > CONTAM_CEIL / 2:
                print(f"  ПРОПУСК {n}: скор {s:.4f} вплотную к бленду {sb_full:.4f} И парный "
                      f"вклад {g1:+.6f} повышен — обе черты утечки сразу")
                excluded[n] = f"скор {s:.4f} у бленда при парном вкладе {g1:+.6f} — контаминация"
                continue
            print(f"  ВНИМАНИЕ {n}: калиброванный скор {s:.4f} близок к бленду {sb_full:.4f}, "
                  f"но парный вклад {g1:+.6f} обычный — беру, проверьте протокол обучения")
        if g1 > CONTAM_CEIL:
