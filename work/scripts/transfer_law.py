...
ОГОВОРКА О SIGMA (добавлена после слияния 23.08). Параболическая sigma(kappa), по которой
здесь делятся точки, ОТОЗВАНА Жениной частью C: она занижает неопределённость в 4-28 раз,
тем сильнее, чем больше q; истинная sigma_kappa = F0/sqrt(n_публики*q). Порог TIGHT ниже
поэтому не «измерена/не измерена», а лишь «параболическая sigma мала». ВЫВОДЫ ЭТО НЕ
МЕНЯЕТ, и это проверено: на обоих наборах класс проигрывает общему среднему (6 точек
0.229 против 0.214; 8 точек 0.454 против 0.338), а опровержение гипотезы 1 вообще не
зависит от sigma — там сравниваются ЗНАКИ на контролируемой паре. Сдвигается только
слепая доза: E[kappa] = 0.20 по шести точкам и 0.248 по всем восьми.
