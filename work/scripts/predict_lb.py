...
MEASURED: list[tuple[str, str, float]] = [
    ...
    # SHOW9/10 — λ-разрез слип-закона на 123-спане: слипы ОТРИЦАТЕЛЬНЫЕ
    # (−0.000045 при Σ|c|=7, −0.000146 при Σ|c|=19) — закон «+1.75e-5·Σ|c|»
    # не универсален: вход SHOW4/5/8 в базис заякорил шумовые направления
    # (механизм «удаления слипа» из Части F §2.4 реализовался сам).
    # SHOW10 — новый лучший паблик. НЕ ФИНАЛИСТЫ.
    ("SHOW9_l1e2",       "SHOW9_l1e2.csv",                 1.6446538955373227),
    ("SHOW10_l3e3",      "SHOW10_l3e3.csv",                1.6446514942),
    #разрез
    # ядра recency + структурные когорты.
    
    
    
    # Расчёт 1.6465664, факт в 2.7e-6. E[private] ≈ +0.00045 к T3-цепочке.
    ("F3_priv",          "F3_priv.csv",                    1.646635897645729),
    ("F4_priv",          "F4_priv.csv",                    1.6468218952),
    #затухающие +0.043±0.058,
    # растущие −0.059±0.067, разжалованные киты −0.039±0.221, декабрьские
    # +0.043±0.119 — ВСЕ НУЛИ. Карта структурных смещений теста исчерпана.
    #промо по источникам (осень-зима
    # +0.168±0.130 слабый, весна +0.058±0.187 ноль), НИКОГДА НЕ ПОКУПАВШИЕ
    # κ=−0.197±0.067 (−2.9σ, переоценка −0.052 лога — зона оракула нулей),
    # корзинщики −0.042±0.066 ноль.
    #все три ОТРИЦАТЕЛЬНЫЕ
    # (−0.169±0.157 / −0.189±0.101 / −0.246±0.157), seg_realgr лёг на линию гипотезы
    # «перенос молчания полный» с точностью 0.1σ. ВЕРДИКТ: молчание недодозировано
    
    # (+7e-5 паблика, ~+1.2e-4 привата). Детали: отчёт агента «Молчащие» 28.08.
    ("P46_silcarrier",   "P46_silcarrier.csv",             1.647434809),
    # F5_priv — полная совместная пересборка 32 осей (когерентный многомерный
    #Алгебра 1.6462419, предиктор
    # 1.6463461, факт между (мини-слип +5.6e-5 по закону). ЛУЧШИЙ законный
    # паблик и лучший E[priv] = +0.000923 к T3. Кандидат финалиста-1.
    ("F5_priv",          "F5_priv.csv",                    1.6462975719),
    #ЧЕТЫРЕ живые оси —
    # фаза цикла +0.138(2.1σ), метрономы −0.150(2.3σ), спящие-браузеры
    # +0.372(3.4σ!), горячие корзины +0.196(3.0σ); на грани burst14 +0.175,
    # cartpend +0.105; стрики ноль. Дозы в F6.
    #Расчёт 1.6459800,
    # факт −3.4e-6. Лучший законный паблик; E[priv] ≈ +0.00107 к T3. Финалист-1.
    ("F6_priv",          "F6_priv.csv",                    1.6459765591),
    
    
    # живые, но при исправленной σ×1.15 (аудит K1) их w_p≈0: дозы не окупаются.
    # Остальные нули. Вопрос локальной σ_u — Жене (K3).
    # mdl_wulfen — зонд оси ktpp (нейро-TPP Кости, вал-вклад +6e-5 подтверждён): κ на
    # паблике −0.240±0.234 — не подтвердилась, доза ≤0.3 шума, ось не применяется.
    # Модель остаётся направлением недели (tabless-доктрина).
    ("N1_ktpp",          "N1_ktpp.csv",                    1.646333227),
    
    #Расчёт 1.6458570,
    # факт в 1.3e-6. ЛУЧШИЙ законный паблик, финалист-1.
    ("F7_priv",          "F7_priv.csv",                    1.6458557351),
    # SHOW11 — оболочка на 163-спане, λ=3e-3, Σ|c|=27: расчёт 1.6438244,
    # слип +0.000182 (закон давал +0.000284 — шестая точка в полосе).
    # Витрина #1 паблика. НЕ ФИНАЛИСТ.
    ("SHOW11_hull4",     "SHOW11_hull4.csv",               1.6440063524),
    # F8_priv — финальная пересборка: 46 осей, локальная σ_u по каждой (правило
    # K1b обобщено), mdl_wulfen исключён (замер важнее приора). Алгебра 1.6457652,
    # факт +4.1e-5 (систематика больших комбо). E[priv] ≈ +0.0014 к T3. Финалист-1.
    ("F8_priv",          "F8_priv.csv",                    1.6458057389),
    # F12_ebint — F8-сборка с расщеплённым сегментным приором (EB): модельная семья
    # tau 0.196->0.2855, сегментный пул 0.1406 -> silent .0575 / cart_promo .1060 /
    # temporal .1120, Z-разложение реестровое. Прогноз ЗАПЕЧАТАН до замера (f12-seal):
    # 1.6456663, 68% [1.645620, 1.645712]. Факт +9.9e-6 от печати (0.45 шума), внутри
    # 68%-полосы. ЛУЧШИЙ ЗАКОННЫЙ ПАБЛИК, обходит T (1.6457820). Кандидат финалиста-1.
    ("F12_ebint",        "F12_ebint.csv",                  1.6456761695614883),
    # F13_g0 — линия A3 при γ=0 (отказ от усадки): тот же приор, что у F9_eb08,
    # k = 11.27 (расширенный tau_model БЕЗ расщепления сегментного — это НЕ γ=0
    # версия F12, у них Λ различается на 21 сегментной оси). Алгебра 1.6456098,
    # прогноз ЗАПЕЧАТАН до замера (58105da): 1.645678, 68% [1.645645, 1.645710].
    # Факт +2.4e-5 от печати (1.11 шума), внутри 68%; |факт-алгебра| = 9.26e-5 при
    # коридоре 1.2e-4 — ПРИНЯТ. Паблик хуже F12 на 1.19 шума, как и предсказано;
    # ценность файла в дисперсии: sd разности пары 3.79 шума. СЛОТ-2.
    ("F13_g0",           "F13_g0.csv",                     1.6457023817),
    # --- ночь 30.08, последние три посылки кампании. Все три собраны из состояний
    # солвера линии A, прогноз запечатан ДО отправки, все три легли в коридор §9.
    # F14_int08 — рецепт F12 с единственным изменением: tau_decomp 0.0148 -> 0.0209
    # (Lam отличается ровно на 9 осях разложения). Прогноз 1.6455437, промах +0.33
    # шума. k = 9.24 против 7.755 у F12; дифференциал ЗАМЕРЕН (та же точка ebz209),
    # 3.561 направления = 5.33 шума штрафа при 5.69 шума выигрыша паблика.
    ("F14_int08",        "F14_int08.csv",                  1.6455509996305273),
    # F17_int10 — тот же ход по приору при gamma 0.10 вместо 0.08. Прогноз
    # 1.6455523, промах те же +0.33 шума. Нетто против F12 +0.03 — неотличим.
    ("F17_int10",        "F17_int10.csv",                  1.645559577558563),
    # F19_seg4 — расщеплённый сегментный приор БЕЗ расширения модельного: k = 4.783,
    # то есть на 2.97 направления МЕНЬШЕ, чем у F12. Паблик хуже на 3.64 шума,
    # кредит по k +4.45 -> нетто -0.81 шума В ПОЛЬЗУ seg4. Первый файл кампании,
    # который покупает приват ценой паблика. Прогноз 1.6457544, промах +0.08 шума.
    ("F19_seg4",         "F19_seg4.csv",                   1.6457561648),
    # F21_seg4g08 — ПОСЛЕДНЯЯ посылка кампании. seg4 с гребнем γ 0.10 -> 0.08.
    # Собран как lp(seg4) + lp(int08) − lp(int10): сборка аффинна по дозам, а int08 и
    # int10 отличаются ровно гребнем, оба замерены. Гребень не входит НИ в Λ, НИ в cQ,
    # значит по признаку сравнимости k не меняется: 4.783, как у seg4. Единственный
    # ход кампании, улучшающий и паблик, и приват при нулевой цене в направлениях.
    # Прогноз 1.6457426, промах +0.23 шума. Нетто против F12 -1.20 шума — лучший
    # файл кампании по приватному ожиданию (seg4 -0.81, int08 -0.36).
    ("F21_seg4g08",      "F21_seg4g08.csv",                1.6457476778206683),
