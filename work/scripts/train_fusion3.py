...
def main():
    ...
    if args.final:
        ...
        # ИСТОРИЯ ФЛАГА --retrain-from-best, чтобы его не включили заново.
        # Наблюдение было такое: переобучение идёт ФИКСИРОВАННОЙ длины и игнорирует
        # точку остановки первой фазы (best_step 492-1968 против 3321 шага), то есть
        # тестовая модель учится в 1.7-6.7 раза дольше валидационно-оптимальной.
        # ЭТО НАБЛЮДЕНИЕ БЫЛО СДЕЛАНО ПО СЫРОЙ МЕТРИКЕ И НЕВЕРНО. Прямой замер с
        # калиброванным критерием показал обратное: калиброванный скор падает до конца
        # бюджета, а сокращение длины даёт НЕДОобученную модель (у rb555 разброс
        # прогноза упал 1.5666 -> 1.4527). KNOWLEDGE: «флаг --retrain-from-best ВРЕДЕН
        # и использовать его нельзя; фиксированная длина переобучения была права».
        # Поэтому он выключен по умолчанию и включать его не надо.
