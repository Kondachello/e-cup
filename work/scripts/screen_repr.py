...
!!! ВНИМАНИЕ, У ЭТОГО СКРИПТА ЕСТЬ ИЗВЕСТНЫЙ ЛОЖНОПОЛОЖИТЕЛЬНЫЙ РЕЖИМ !!!
Разрез по половинам пользователей внутри ОДНОГО окна контролирует переобучение
по пользователям, но НЕ по окну. Недельное представление показывало здесь R^2
до 0.0012, а при разносе окон на 30+ дней тот же замер даёт -0.031, то есть
сигнал был свойством окна, а не данных. Это то же семейство, что известный
«перенос на тест 2%».

ОБЯЗАТЕЛЬНО после положительного результата здесь прогоняй проверку переноса:
оцени представление на остатках ОДНОГО среза и замерь на срезе, отстоящем
минимум на 30 дней (целевые окна не должны пересекаться). Готовый источник
честных остатков без обучения — work/features/anchor=*.seqoof.parquet.
Положительный результат тут без положительного переноса не значит ничего.


Логика. ВНИМАНИЕ: правило 7.1*delta^2 ниже похоронено 20-21.08 (занижает сильные модели
в 15-20 раз); действующая приёмка — margin.py / joint_gain.py. Само тождество для ЗАПАСА
в силе. Доказано тождество: вклад модели в бленд равен примерно 7.1*delta^2, где
