...
ПАРА ФИНАЛИСТОВ НА 30.08, вечер (см. FINALISTS ниже): F12_ebint (слот 1) + F13_g0 (слот 2).
F13_g0 замерен 1.6457023817 и принят по коридору; T3 вытеснен — на настоящей доске он
давал +0.00 п.п. P(топ-3), а F13_g0 даёт +0.46 против F8 и +0.53 против T3.
F8_priv вытеснен из слота-1 штатно по §9 finalists.md: F12 замерен 1.6456761695614883,
лучше F8 на 5.9 шума, промах от запечатанной печати +9.9e-6 (0.45 шума).
...
FINALISTS: dict[str, dict] = {
    "F12_ebint": {
        "file": "F12_ebint.csv",
        "slot": 1,
        "pub": 1.6456761695614883,
        # k(F12) = k(F8) + Δk. Δk считается дифференциально по route D
        # (k = Σ 1.25·w_i − 0.25, w_i = tau_i²/(tau_i²+σ_i²)): модельная семья
        # tau 0.196->0.2855 даёт +2.97, сегментные tau ВСЕ сужаются
        # (0.141 -> 0.0575/0.1060/0.1120) и дают от −1.98 до −8.56.
        # Δk ≤ +0.99 при ЛЮБОМ распределении осей по подгруппам; порог
        # безубытка против F8 — Δk > 3.94. Берём пессимистический край.
        "k": 9.3,
        "sha256": "6b905c2d5d3818aef15daf192f5aabbb23898dc12546639f5e031c9d040f2d8f",
        "note": "F8 + расщеплённый сегментный приор; печать прогноза попала в 0.45 шума",
    },
    "F8_priv": {
        "file": "F8_priv.csv",
        "slot": 1,
        ...
        "note": "пересборка 46 осей, локальная σ_u; вытеснен F12 по §9 (30.08), резерв слота-1",
    },
    "F13_g0": {
        "file": "F13_g0.csv",
        "slot": 2,
        "pub": 1.6457023817,
        # k линии A3: k(F8)=8.3 + расширение tau_model 0.196->0.2855 (+2.97).
        # Расщепления сегментного приора здесь НЕТ, поэтому вычитать нечего и
        # k выше, чем у F12. Это НЕ γ=0-версия F12: Λ различается на 21 оси.
        "k": 11.27,
        "sha256": "ebf7883a46ed6ecee3eca8080ff4d9f9e1180c65b6e34a99887f7c64c12d7626",
        "note": "γ=0, отказ от усадки; принят по коридору 9.26e-5 <= 1.2e-4; "
                "слот-2 как лотерейный билет: sd разности пары 3.79 шума",
    },
    "T3_g1_redose_044": {
