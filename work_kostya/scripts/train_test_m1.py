...
ALL = json.load(open("/root/work/testgrid_days.json"))
TRAIN = ALL[-7:]  # 332..374
TEST_DAY = 409
...
Xte = np.load("/root/work/Xtest_409.npy")[:, :121]
...
pz = reg.predict(Xte); reg.save_model("/root/work/mt1_reg.txt"); del reg; gc.collect()
...
p = clf.predict(Xte); clf.save_model("/root/work/mt1_clf.txt"); del clf; gc.collect()
...
s = size.predict(Xte); size.save_model("/root/work/mt1_size.txt"); del size; gc.collect()
print("size done", flush=True)
np.save("/root/work/mt1_test_pz.npy", pz.astype(np.float32))
np.save("/root/work/mt1_test_two.npy", (p * s).astype(np.float32))
