...
LOCAL ADAPTATION (delta vs ../scripts/train_model.py):
1) /root/work -> <repo>/work_kostya/work;
2) seed of the `common` dict parameterized via env KSEED (unset -> original 1).
No other changes.
"""
...
np.save(f"{_W}/Xval2_379.npy", Xv)
json.dump(names, open(f"{_W}/feat_names2.json", "w"))
print("train matrix", Xtr.shape, "seed", _SEED1, flush=True)
...
size.save_model(f"{_W}/size_z.txt")
...
np.save(f"{_W}/val_pz.npy", pz); np.save(f"{_W}/val_two.npy", two)
np.save(f"{_W}/val_p.npy", p); np.save(f"{_W}/val_s.npy", s)
